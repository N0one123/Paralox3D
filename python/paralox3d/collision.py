...
class Collider:
    ...
    def __init__(self,owner,size=None,offset=(0,0,0),enabled=True,is_trigger=False,layer="default",collides_with=None):
        ...
        self.collides_with=set(collides_with or {"*"})
    ...
    def min(self):
        p=self.position
        return Vec3(p.x-abs(self._size.x)/2,p.y-abs(self._size.y)/2,p.z-abs(self._size.z)/2)
    ...
    def sync_to_scale(self):
        if self._auto_size:
            self._size=self.owner.scale.copy()

def _compatible(a,b):
    return a.enabled and b.enabled and (
        "*" in a.collides_with or "*" in b.collides_with
        or b.layer in a.collides_with or a.layer in b.collides_with
    )
...
def _dispatch_collision_events(engine):
    current=set()
    objects=tuple(engine._objects)
    for i,a in enumerate(objects):
        if not a.enabled or not a.collider.enabled:
            continue
        for b in objects[i+1:]:
            if not b.enabled or not b.collider.enabled or not _overlaps(a.collider,b.collider):
                continue
            pair=(min(a.id,b.id),max(a.id,b.id))
            current.add(pair)
            if pair not in engine._collision_pairs:
                if getattr(a,"on_enter",None): a.on_enter(b)
                if getattr(b,"on_enter",None): b.on_enter(a)
            if getattr(a,"on_collision",None): a.on_collision(b)
            if getattr(b,"on_collision",None): b.on_collision(a)
    for pair in engine._collision_pairs-current:
        a=next((o for o in objects if o.id==pair[0]),None)
        b=next((o for o in objects if o.id==pair[1]),None)
        if a is not None and getattr(a,"on_exit",None): a.on_exit(b)
        if b is not None and getattr(b,"on_exit",None): b.on_exit(a)
...
def _ray(origin,direction,mn,mx,distance):
    ...
    for axis,(o,d,a,b) in enumerate(((origin.x,direction.x,mn.x,mx.x),(origin.y,direction.y,mn.y,mx.y),(origin.z,direction.z,mn.z,mx.z))):
        if abs(d)<1e-8:
            ...
            if u>t0:
                t0=u
                normal=Vec3(-1 if d>0 else 1,0,0) if axis==0 else Vec3(0,-1 if d>0 else 1,0) if axis==1 else Vec3(0,0,-1 if d>0 else 1)
            t1=min(t1,v)
...
def boxcast(center,size,direction,distance=1,ignore=()):
    c=center if isinstance(center,Vec3) else Vec3(*center)
...
def spherecast(center,radius,direction,distance=1,ignore=()):
    return boxcast(center,(radius*2,radius*2,radius*2),direction,distance,ignore)
