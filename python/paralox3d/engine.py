...
class Engine:
    ...
    def _configure_abi(self):
        ...
        self._native.p3d_camera_set_enabled.argtypes=[ctypes.c_void_p,ctypes.c_int]
    ...
    def start(self,update=None):
        ...
        while self._running:
            ...
            self._native.p3d_camera_set_enabled(engine_ptr, int(camera_enabled))
            if camera_enabled and (camera._dirty or not self._camera_active):
                self._native.p3d_camera_set_transform(
                    engine_ptr, camera.x, camera.y, camera.z,
            ...
            _default_input._sync(lambda key_code: self._native.p3d_input_key_held(engine_ptr, key_code))
    ...
    def close(self):
        global _default_engine
        if self._engine:
            self._native.p3d_engine_destroy(ctypes.c_void_p(self._engine)); self._engine=None; self._running=False
