...
class FPSController(Component):
    def __init__(self,owner=None,speed=5.0,sprint=8.0):
        super().__init__(owner)
        self.speed=speed
        self.sprint=sprint
    ...
    def update(self):
        ...
        if move.length():
            self.owner.translate(*(move.normalized()*(self.sprint if held("shift") else self.speed)*float(dt)))

class CharacterController(FPSController):
    def __init__(self,owner=None,speed=5.0,jump_speed=6.0,gravity=-18.0):
        super().__init__(owner,speed)
    ...
    def update(self):
        ...
        if pressed("space") and self.grounded:
            ...
            self.grounded=False
        self.velocity_y+=self.gravity*float(dt)
        self.owner.y+=self.velocity_y*float(dt)
        ...
        for o in self.owner._engine._objects:
            if o is not self.owner and not o.collider.is_trigger and Collision(self.owner,o) and self.velocity_y<=0:
                self.owner.y=o.collider.max.y+self.owner.collider.size.y*.5
                self.velocity_y=0
                self.grounded=True
